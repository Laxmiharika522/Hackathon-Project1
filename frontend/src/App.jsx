...
function loadStats() {
  try {
    return JSON.parse(localStorage.getItem('cyberguard_stats')) || {
...
export default function App() {
  ...
  const handleResult = useCallback((result) => {
    setStats(prev => {
      ...
      localStorage.setItem('cyberguard_stats', JSON.stringify(next))
      return next
    })
  }, [])
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <div style={{ fontSize: 14, fontWeight: 700 }}>
              <span style={{ color: '#00ff88' }}>Cyber</span>Guard AI
            </div>
